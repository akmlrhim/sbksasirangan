<x-layouts :title="$title">
  <main class="relative w-full overflow-hidden bg-third pt-28 pb-16 sm:pt-36 sm:pb-20 md:pt-44 md:pb-32">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-10 lg:gap-16 items-start mb-16 sm:mb-20">
        <div class="md:col-span-7 space-y-6 sm:space-y-8" data-aos="fade-right">
          <div class="space-y-2">
            <h1 class="font-header text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-primary leading-[1.1]">
              {{ __('Reni Andrina') }} <br>
              <span class="text-[#eec04b] italic font-header">{{ __('Rahmawati') }}</span>
            </h1>
          </div>
          <div class="grid grid-cols-1 gap-4 sm:gap-5 pt-2">
            <div
              class="group flex items-start gap-3 sm:gap-4 p-4 sm:p-5 rounded-2xl bg-white/50 border border-gray-100 shadow-sm transition-all hover:shadow-md">
              <div
                class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-primary text-white flex items-center justify-center shadow-sm">
                <i class="fa-solid fa-location-dot text-base sm:text-lg"></i>
              </div>
              <div>
                <h3 class="text-primary font-bold text-base sm:text-lg mb-1">{{ __('Region') }}</h3>
            ...
            <div
              ...
              <div>
                <h3 class="text-primary font-bold text-base sm:text-lg mb-1">{{ __('Affiliations & Roles') }}</h3>
                <p class="text-gray-600 text-xs sm:text-sm md:text-base font-medium leading-relaxed">
          ...
          <div class="flex flex-wrap items-center gap-3 sm:gap-4 pt-4">
            <div class="flex items-center gap-2 sm:gap-3">
              <a href="#"
                class="w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white border border-primary/20 text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all shadow-sm"><i
                  class="fa-brands fa-instagram text-base sm:text-lg"></i></a>
              <a href="#"
                class="w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white border border-primary/20 text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all shadow-sm"><i
                  class="fa-brands fa-linkedin-in text-base sm:text-lg"></i></a>
            </div>
            <div class="hidden sm:block w-px h-8 bg-gray-300 mx-2"></div>
            <a href="mailto:reni@sasiranganbanjar.com"
              class="flex items-center gap-2 sm:gap-3 px-5 py-2.5 sm:px-7 sm:py-3 bg-[#eec04b] text-white rounded-full shadow-lg hover:bg-[#d4a837] transition-all">
              <i class="fa-regular fa-envelope text-sm sm:text-base"></i>
              <span class="font-bold text-xs sm:text-sm tracking-wide">reni@sasiranganbanjar.com</span>
            </a>
          </div>
        </div>
        <div class="md:col-span-5 relative" data-aos="fade-left">
          <div class="relative w-full rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl aspect-[4/5]">
      ...
      <div class="border-t border-gray-200 pt-12 sm:pt-16" data-aos="fade-up">
        <h2 class="font-header text-2xl sm:text-3xl md:text-4xl font-bold text-primary mb-6 sm:mb-8">
          {{ __('Biography') }}</h2>
        <div
          class="prose prose-sm sm:prose-base md:prose-lg prose-headings:text-primary text-gray-600 leading-relaxed max-w-none text-justify space-y-4 sm:space-y-6">
          <p>
      ...
      <section class="py-16 sm:py-24">
        ...
        <div class="mb-16 sm:mb-24" x-data="{
            atBeginning: true,
            atEnd: false,
            next() {
          ...
          <div class="flex items-center justify-between mb-8 sm:mb-10">
            <div class="flex items-center gap-3 sm:gap-4 flex-1">
              <h3 class="text-xl sm:text-2xl font-header font-bold text-primary">{{ __('Management Team') }}</h3>
            ...
            <div class="flex gap-2 ml-4">
              <button @click="prev()" :disabled="atBeginning"
                ...
                class="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-primary text-primary flex items-center justify-center transition-all">
          ...
          <div x-ref="container" @scroll.debounce.50ms="check()"
            ...
            @forelse ($management as $member)
              <div class="snap-start flex-shrink-0 w-full md:w-[calc(25%-1.5rem)] md:mr-8 group"
                ...
                <div class="relative overflow-hidden rounded-2xl aspect-[3/4] mb-4 sm:mb-5 shadow-sm bg-gray-100">
                  ...
                  @else
                    <div
                      ...
                      <i class="fa-solid fa-user text-4xl sm:text-5xl text-gray-400"></i>
                    </div>
                  @endif

                  <div x-show="showBio" x-transition class="absolute inset-0 bg-white/98 z-30 p-5 sm:p-6 flex flex-col"
                    style="display: none;">
                    <button @click="showBio = false" class="self-end text-gray-400 hover:text-primary"><i
                        class="fa-solid fa-xmark text-xl sm:text-2xl"></i></button>
                    <h4 class="font-header text-lg sm:text-xl text-center text-primary font-bold mb-3 sm:mb-4">
                      {{ $member->name }}</h4>
                    <div
                      class="flex-1 overflow-y-auto text-xs sm:text-sm text-gray-600 leading-relaxed scrollbar-hide text-center">
                ...
                <div class="text-center">
                  <h3 class="text-base sm:text-lg font-bold text-primary mb-2 sm:mb-3">
                    ...
                    <div class="flex gap-3">
                      @if ($member->ig_link)
                        <a href="{{ $member->ig_link }}" class="text-primary/50 hover:text-primary"><i
                            class="fa-brands fa-instagram text-lg sm:text-xl"></i></a>
                      @endif
                      @if ($member->linkedin_link)
                        <a href="{{ $member->linkedin_link }}" class="text-primary/50 hover:text-primary"><i
                            class="fa-brands fa-linkedin-in text-lg sm:text-xl"></i></a>
                      @endif
                    </div>
                  </div>
                </div>
            ...
            @empty
              <p class="w-full text-center text-sm sm:text-base text-gray-400">{{ __('No team members found.') }}</p>
            @endforelse
          </div>
        </div>
        ...
        <div x-data="{
          ...
          <div class="flex items-center justify-between mb-8 sm:mb-10">
            <div class="flex items-center gap-3 sm:gap-4 flex-1">
              <h3 class="text-xl sm:text-2xl font-header font-bold text-primary">{{ __('Our Artisans') }}</h3>
              <div class="h-px flex-1 bg-gray-200"></div>
            </div>
            <div class="flex gap-2 ml-4">
              <button @click="prev()" :disabled="atBeginning"
          ...
          <div x-ref="container" @scroll.debounce.50ms="check()"
            ...
            @forelse($artisans as $artisan)
              <div class="snap-start flex-shrink-0 w-full md:w-[calc(25%-1.5rem)] md:mr-8 group"
                ...
                <div class="relative overflow-hidden rounded-2xl aspect-[3/4] mb-4 sm:mb-5 shadow-sm bg-gray-100">
                  ...
                  <div x-show="showBio" x-transition
                    class="absolute inset-0 bg-white/98 z-30 p-5 sm:p-6 flex flex-col" style="display: none;">
                    <button @click="showBio = false" class="self-end text-gray-400 hover:text-primary"><i
                        class="fa-solid fa-xmark text-xl sm:text-2xl"></i></button>
                    <h4 class="font-header text-lg sm:text-xl text-center text-primary font-bold mb-3 sm:mb-4">
                      {{ $artisan->name }}</h4>
            ...
            @empty
              <p class="w-full text-center text-sm sm:text-base text-gray-400">{{ __('No artisans found.') }}</p>
            @endforelse
          </div>
        </div>
      </section>
