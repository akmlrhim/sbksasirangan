    ...
    <section class="w-full flex flex-col md:flex-row min-h-[300px] sm:min-h-[400px] md:h-[600px] pt-16 sm:pt-20">
      <div
        ...
        <div class="max-w-lg z-10">
          <span
            class="text-secondary tracking-[0.2em] text-[10px] sm:text-xs md:text-sm font-bold uppercase mb-1.5 sm:mb-2 block">
            {{ __('Documentation') }}
          </span>
          <h1 class="text-white text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight leading-tight">
            {{ __('Our Gallery') }}
          </h1>
          <p
            class="mt-3 sm:mt-4 md:mt-6 text-white font-medium text-sm sm:text-base md:text-lg tracking-wide font-light leading-relaxed">
    ...
    <section
      ...
      <div
        ...
        <div class="relative flex-shrink-0">
          <h2 class="text-2xl sm:text-3xl md:text-4xl text-primary font-bold">{{ __('Latest Archives') }}</h2>
        </div>
        ...
        <div class="w-full md:w-auto overflow-x-auto pb-2 scrollbar-hide">
          <div class="flex flex-nowrap gap-x-4 sm:gap-x-6 md:gap-x-8">
            <button @click="activeTab = 'all'"
              class="whitespace-nowrap text-xs sm:text-sm capitalize transition-all duration-300 pb-1 border-b-[1.5px] sm:border-b-2 outline-none"
      ...
      <div x-show="activeTab !== 'all'" class="mb-8 sm:mb-12 min-h-[50px] sm:min-h-[60px]">
        @foreach ($galleries as $descItem)
          <div x-show="activeTab === '{{ (string) $descItem->id }}'"
            ...
            <h3 class="text-xl sm:text-2xl font-bold text-primary">{{ $descItem->title }}</h3>
      ...
      <div class="relative w-full min-h-[300px] sm:min-h-[400px]">
        ...
        <div x-show="isLoading"
          class="absolute inset-0 z-40 flex flex-col items-center justify-start pt-16 sm:pt-20 bg-blue-50">
          <div class="w-10 h-10 sm:w-12 sm:h-12 border-4 border-primary/10 border-t-primary rounded-full animate-spin">
          </div>
          <span class="mt-3 sm:mt-4 text-[10px] sm:text-xs font-bold tracking-widest text-primary animate-pulse">
            {{ __('LOADING...') }}
          </span>
        </div>
        ...
        <div x-show="!isLoading">
          @if ($galleries->isEmpty())
            <div
              ...
              <p class="text-gray-400 tracking-wide text-xs sm:text-sm">
    ...
    <div x-show="lightboxOpen" x-transition:enter="transition ease-out duration-300"
      ...
      <div class="relative max-w-5xl w-full flex flex-col items-center" @click.stop>
        ...
        <p x-text="lightboxTitle"
          class="mt-4 sm:mt-6 text-white/90 text-sm sm:text-base md:text-lg tracking-widest uppercase text-center font-bold">
        </p>
      </div>
    </div>
